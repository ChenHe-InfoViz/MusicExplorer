...
function initData(ctx, callback) {
    ...
    dataForVis = new dataParser(changeDataset);

    //$.when($.ajax({ url: './javascripts/datasets.json', dataType: 'json' })).then(
...
var changeDataset = function () {
    ...
    $(EventManager).trigger("loading-dataset-started", {
        description: dataForVis.JSONObject
    }
    //dataSetDescriptions[queryParameters['dataset']]
    );
...
function processDataSet() {
    //d3.text(dataSetDescription.file, 'text/csv', //function (data) {
        parseDataSet(dataForVis.csvContent, dataForVis.JSONObject);
        run();
    //});
}
...
function retrieveQueryParameters() {
    ...
    //queryParameters['userid'] = queryParameters['userid'] || 16;
...
//chen: run one time first loaded
function run() {
...
//parsing csv file according to json; data: csv;
function parseDataSet(data, dataSetDescription) {
    ...
    for (var i = 0; i < dataSetDescription.sets.length; ++i) {
        var setDefinitionBlock = dataSetDescription.sets[i];
        ClusterNames.push(dataSetDescription.sets[i].name);
        if (setDefinitionBlock.format === 'binary') {
    ...
    for (var i = 0; i < rawSets.length; i++) {
        ...
        combinedSets[i] = 1;
        var set = new Set(setPrefix + i, setNames[i], combinedSets, rawSets[i], setClusters[i]);
        setIdToSet[setPrefix + i] = set;
        sets.push(set);
        if (set.cluster == 0) {
            set.isSelected = true;
            usedSets.push(set);
            //clusterTemp = setClusters[i];
            //console.log(setClusters[i]);
        }
        //other users cluster
        if(setClusters[i] == 1) {
            var names = setNames[i].split("(");
            //console.log(dataForVis.UserAndBookmarks);
            for(j = 0; j < dataForVis.UserAndBookmarks.length; j++)
            {
                if(dataForVis.UserAndBookmarks[j]["userName"] == names[0].trim())
                {
                    dataForVis.UserAndBookmarks[j]['setID'] = setPrefix + i;
                    break;
                }
            }
            ...
            if (globalname == names[0].trim()) {
                globalSetId = setPrefix + i;

                if (usedSets.indexOf(set) < 0) {
                    set.isSelected = true;
                    usedSets.push(set);
                }
            }
    ...
    usedSets.forEach(function(set){setInUse.push({name: set.elementName, type: set.cluster})});
    userBehavior.push({
        type: 0,
        usedSets: setInUse,
...
//set up the selected sets for set visualization
...
//chen: update set selection
function updateSetContainment(set, refresh) {
    //console.log("update set containment");
    if (!set.isSelected) {
        set.isSelected = true;
        usedSets.push(set);
        $(EventManager).trigger("set-added", { set: set });
        document.getElementById("tipbar").innerHTML = "[ " + set.elementName + " ] is added to the set view.";
    }
    else {
        ...
        if (index > -1) {
            usedSets.splice(index, 1);//delete
            $(EventManager).trigger("set-removed", { set: set });
            document.getElementById("tipbar").innerHTML = "[ " + set.elementName + " ] is removed from the set view.";
        }
    }
    ...
    userBehavior.push({
        type: 1,
        set: [{name: set.elementName, type: set.cluster}],
        check: set.isSelected,
        usedSets: setInUse,
        time: (date.getDate() < 10 ? '0' : '') + date.getDate() + " @ " + (date.getHours() < 10 ? '0' : '') + date.getHours() + ":" + (date.getMinutes() < 10 ? '0' : '') + date.getMinutes() + ":" + (date.getSeconds() < 10 ? '0' : '') + date.getSeconds()
    });
...
//        plotSetSelection();
        plotSetOverview();
        //console.log("call from containment");
        initCallback.forEach(function (callback) {
            callback();
        })
