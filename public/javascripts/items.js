...
function plotSelectedItems( elementId, selection ) {
    ...
    var headinfo = []; headinfo.push("name"); headinfo.push("artist"); headinfo.push("status");
...
function drawTableRows(tbody, trackinfo, selection){
    tbody.selectAll("tr")
            ...
            .each(function(d,i) {
            var info = [];
            info.push(d.name); info.push(d.artist); info.push(d.url);
                d3.select(this).selectAll("td")
                    ...
                    if(j == 0) //title
                    {
                        d3.select(this).append("a")
                            ...
                            .on("click", function(){
                                ...
                                var date = new Date();
                              userBehavior.push({type: 5, item: d.name,time: (date.getDate()<10?'0':'') + date.getDate()  + " @ " + (date.getHours()<10?'0':'') + date.getHours()  + ":" + (date.getMinutes()<10?'0':'') + date.getMinutes()  + ":" + (date.getSeconds()<10?'0':'') + date.getSeconds()});
                    ...
                    else d3.select(this).text(function(a) {return a});
                })
            });
}