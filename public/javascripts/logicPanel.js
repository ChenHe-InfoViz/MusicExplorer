...
function LogicPanel(params){
    ...
    var belowCol = params.belowCol;     //the columns to be extended
        ...
        logicExpression = {
            ...
            getListOfValues:function(){
                ...
                this.orClauses.forEach(function(orClause){
                    ...
                    var compareObject = []
                    for (key in orClause){//key is set id
                        compareObject.push(orClause[key].state);
                    }
                    compareList.push(compareObject)
                })
            ...
            elements: function(){
                ...
                this.orClauses.forEach(function(orClause){
                    compareObject = [];
                    for (key in orClause){
                        compareObject.push({key: key, state: orClause[key].state});
                    }
                    all.push(compareObject);
                    //compareList.push(compareObject)
                })
                return all;
            }
...
//            .on("click", addLogic)
        ...
        //chen: the green plus button at table header
        addLogicRect = addLogicButton.append("rect").attr({
            width:20,
            height:20,
            x:0,
            y:0,
        ...
        addLogicRect.style({fill: logicColor, "cursor":"pointer", opacity:.5 })
            .on("click", addLogic)
...
//query selection changed: must, maybe, not
...
//                console.log(d);
        ...
        infoBar.transition().attr({
            width:(function(d){return ctx.subSetSizeScale(d.count)})
        })
...
//                stroke:grays[1],
    ...
    if (isExpanded){
        ...
        textDescriptionPanel.enter().append("text").attr({
            ...
            "transform":"translate("+ (120 + ctx.xStartSetSizes) + ", 0)",//(noOfSets*cellWidth+5+90)+","+0+")",
            y:cellSize,
            dy:1
        }).style({
                "text-anchor":"start",
        ...
        textDescriptionPanel.text(function(d){return d}).call(wrap,width-((ctx.xStartSetSizes+90+20)));
...
//        console.log("clist:",clausesList);
        ...
        tableRows.attr({
            "transform":function(d,i){return "translate("+(120)+","+
...
//        })
    ...
    function submitExpression() {
        try{
            var breakException = {};
            logicExpression.orClauses.forEach(
                    function(orClause) {
                        //console.log(i);
                        if(Object.keys(orClause).length != usedSets.length) {
                            //console.log("length" + orClause.length);
                            throw breakException;
                        }
                        for(var key in orClause)
                        {
                            var has = false;
                            usedSets.forEach(function(d){
                                if(d.id == key)
        ...
        //console.log(usedSets.length);
        //console.log(logicExpression.getListOfValues())

        var date = new Date();
        var setState = [];
        logicExpression.orClauses.forEach(function(con) {
            var oneState = [];
            for(key in con) {
                //console.log(key);
                oneState.push({set: setIdToSet[key].elementName, type: setIdToSet[key].cluster, state: con[key].state});
            }
            setState.push(oneState);
        });
        userBehavior.push({type: 6, setState: setState, time: (date.getDate()<10?'0':'') + date.getDate()  + " @ " + (date.getHours()<10?'0':'') + date.getHours()  + ":" + (date.getMinutes()<10?'0':'') + date.getMinutes()  + ":" + (date.getSeconds()<10?'0':'') + date.getSeconds()});
...
//
        ...
        self.isShown = true;
        self.panelHeight = endOfPanel + 15;
        ctx.svgHeight = renderRows.length * (ctx.cellDistance+4) + self.panelHeight;
        ctx.svgBody.attr({height: ctx.svgHeight});
        //console.log("panel height: " + self.panelHeight + "  " + ctx.svgHeight);
//        belowVis.transition().attr({
//            "y":+(endOfPanel+belowVisRestoreTranslate+15)
//        })
        ctx.updateColumnBackgrounds();
    }
...
//        })
        ...
        addLogicRect.on("click", addLogic);
        ctx.svgHeight = renderRows.length * (ctx.cellDistance + 4);
        ctx.svgBody.attr({height: ctx.svgHeight});
        //console.log("destory: " + ctx.svgHeight);
        ctx.updateColumnBackgrounds();
        initLogicExpression();
    }
